...
export function registerDinerRoutes(router: Router): void {
  router.get("/api/diners/:dinerId/points", async (req, res) => {
    try {
  ...
  router.get("/api/diners/:dinerId/restaurants/:restaurantId/transactions", async (req, res) => {
    try {
  ...
  router.get("/api/diners/:dinerId/transactions", async (req, res) => {
    try {
      const { dinerId } = req.params;
  ...
  router.get("/api/diners/:dinerId/vouchers", async (req, res) => {
    try {
      const { dinerId } = req.params;
  ...
  router.post("/api/diners/:dinerId/vouchers/:voucherId/select", async (req, res) => {
    try {
  ...
  router.post("/api/diners/:dinerId/restaurants/:restaurantId/redeem-credit", async (req, res) => {
    try {
  ...
  router.patch("/api/users/:userId/profile", async (req, res) => {
    try {
  ...
  router.post("/api/phone-change/request", smsRateLimiter, async (req, res) => {
    try {
      if (!req.session.userId) {
  ...
  router.post("/api/phone-change/verify", async (req, res) => {
    try {
      if (!req.session.userId) {
        return res.status(401).json({ error: "Not authenticated" });
      }
